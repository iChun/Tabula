    ...
    public static class ViewNewProject extends View<WindowNewProject>
    {
        public ViewNewProject(@Nonnull WindowNewProject parent)
        {
            ...
            ElementButton button1 = new ElementButton(this, I18n.format("gui.done"), elementClickable ->
            {
                ...
                ((WorkspaceTabula)getWorkspace()).mainframe.openProject(project);
                ((WorkspaceTabula)getWorkspace()).projectChanged(IProjectInfo.ChangeType.PROJECTS);
                ((WorkspaceTabula)getWorkspace()).setCurrentProject(((WorkspaceTabula)getWorkspace()).mainframe.getActiveProject());
                getWorkspace().setFocused(null);
                getWorkspace().removeWindow(parent);
            });
